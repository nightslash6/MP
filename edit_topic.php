...
// Fetch topic data
$stmt = $conn->prepare("SELECT python_id, topic, content, example, question, answer FROM python WHERE python_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        elseif (!empty($topic_answer) && strlen($topic_answer) > 65535) {
            ...
            // Check for duplicate topic (excluding current one)
            $stmt = $conn->prepare("SELECT python_id FROM python WHERE topic = ? AND python_id != ?");
            $stmt->bind_param("si", $topic_title, $topic_id);
            $stmt->execute();
            $result = $stmt->get_result();

            if ($result->num_rows > 0) {
                $message = 'Another topic already has this title! Please choose a different title.';
            } else {
                // Update topic
                $stmt = $conn->prepare("UPDATE python SET topic = ?, content = ?, example = ?, question = ?, answer = ? WHERE python_id = ?");
                $stmt->bind_param("sssssi", $topic_title, $topic_content, $topic_example, $topic_question, $topic_answer, $topic_id);
                
                if ($stmt->execute()) {
                    $_SESSION['message'] = ['successful' => 'Topic updated successfully!'];
                    header('Location: admin_python.php');
                    exit;
                } else {
