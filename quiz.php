...
<head>
    ...
    <style>
        ...
        .powerup-btn:hover {
            background: linear-gradient(135deg, #ffcc02 0%, #ffe066 100%);
            transform: translateY(-1px);
...
<body>
    ...
    <nav class="navbar">
        ...
        <div class="nav-links">
            ...
            <!-- Points Display -->
            <div class="nav-points-shop" style="display:inline-block; margin-left:20px;">
                ...
                fetch('get_shop_items.php')
                    ...
                    .then(data => {
                        ...
                        if (data.success && data.powerups && data.powerups.length > 0) {
                            let html = `<div class="mb-3"><b>Your Points:</b> <span class="badge bg-success">${data.user_points}</span></div>`;
                            ...
                            data.powerups.forEach(item => {
                                ...
                                <div class="list-group-item d-flex justify-content-between align-items-center">
                                    ...
                                    <div>
                                        <span class="badge bg-warning text-dark mb-2">${item.cost} pts</span><br>
                                        <button class="btn btn-sm btn-primary buy-powerup-btn" data-id="${item.powerup_id}" ${data.user_points < item.cost ? 'disabled' : ''}>Buy</button>
                                    </div>
                                </div>`;
                            });
                            html += `</div>`;
